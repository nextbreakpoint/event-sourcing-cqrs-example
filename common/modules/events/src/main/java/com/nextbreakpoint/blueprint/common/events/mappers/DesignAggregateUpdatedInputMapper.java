...
@Log4j2
public class DesignAggregateUpdatedInputMapper implements Mapper<InputMessage, DesignAggregateUpdated> {
    @Override
    public DesignAggregateUpdated transform(InputMessage message) {
        if (!message.getValue().getType().equals(DesignAggregateUpdated.TYPE)) {
