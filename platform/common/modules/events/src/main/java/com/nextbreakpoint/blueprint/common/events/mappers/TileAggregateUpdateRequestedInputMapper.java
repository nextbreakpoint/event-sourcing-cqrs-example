...
public class TileAggregateUpdateRequestedInputMapper implements Mapper<Message, TileAggregateUpdateRequested> {
    ...
    public TileAggregateUpdateRequested transform(Message message) {
        if (!message.getPayload().getType().equals(MessageType.TILE_AGGREGATE_UPDATE_REQUESTED)) {
            throw new IllegalArgumentException("Unexpected message type: " + message.getPayload().getType());
        }
        try {
            return Json.decodeValue(message.getPayload().getData(), TileAggregateUpdateRequested.class);
