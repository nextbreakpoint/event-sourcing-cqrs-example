...
public class S3Driver {
    ...
    public Single<Void> putObject(String key, byte[] data) {
        return Single.fromCallable(() -> putContent(key, data)).map(response -> null);
    }
    ...
    private PutObjectResponse putContent(String key, byte[] data) {
        final Span objectSpan = tracer.spanBuilder("Put object " + key).startSpan();

        try (Scope scope = objectSpan.makeCurrent()) {
            final Span span = Span.current();

            span.setAttribute("object.key", key);
            span.setAttribute("object.size", data.length);

            return s3AsyncClient.putObject(makePutRequest(key), AsyncRequestBody.fromBytes(data)).get();
        } catch (Exception e) {
            throw new RuntimeException(e);
        } finally {
            objectSpan.end();
