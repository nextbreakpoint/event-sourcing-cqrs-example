...
helm upgrade --install integration-consul helm/consul -n platform --set servicePort=8000,serviceName=$(minikube ip)
helm upgrade --install integration-minio helm/minio -n platform
helm upgrade --install integration-nginx helm/nginx -n platform --set hostname=$(minikube ip)
